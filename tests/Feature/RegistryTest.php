...
it('ships a component for every slot it declares', function () {
    ...
    // with no file behind it is a `--replace` that cannot fall back.
    $registry = registry();
...
it('ships three icons that are not slots, and nothing else', function () {
    ...
    // alone by `--replace`. They are prefixed regardless, so that `trash` and
    // `plus` stay free for whatever an application generates for itself.
